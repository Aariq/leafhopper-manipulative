...
Two files have abnormally low values for the peak area of the internal standard.  I'll remove those two samples.
...
weirdos <- gc_IS %>% filter(IS < 100000)
# JGY 11 and JGY 4
...
gc_tidy.2 <- gc_tidy.2 %>% 
  ...
  filter(!filename %in% weirdos$filename)
```
...
Nondetects were zeros to begin with, not found by IonAnalytics at all.  I'm not censoring peaks. However, if RPA <=0 because of non-detect or blank-subtraction, I'll set RPA to a small number (100/IS). This will be useful for log-transforming data since log(0) = -inf. I also add a "present" column that will be useful for getting compound counts and such later on.
...
gc_tidy.7 <-
  gc_tidy.6 %>%
  mutate(RPA = case_when(ND       ~ 100/IS,
                         RPA <= 0 ~ 100/IS, #if after background subtraction peak areas < 500
                         TRUE     ~ RPA),
...
gc_zeroes.final <- gc_zeroes.final %>% filter(!Compound %in% lowQs)
```
...
L15 and J16 are maybe an extreme values, but not worth removing.
