...
gc_IS <- 
  gc_IS %>%
  group_by(cultivar) %>%
  mutate(IS = ifelse(IS < 100000, mean(IS), IS))
...
gc_tidy.final <-
  ...
  filter(sum(present) >= 5) #calculate the number of rows where RPA > 0 then filter the dataframe to keep only those where that sum is greater than or equal to 3
