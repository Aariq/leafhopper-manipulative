...
# Read them all in as dataframes and name them
dflist_2018 <-
...
```{r}
raw_2018 <-
  map2_dfr(.x = dflist_2018, .y = names(dflist_2018), ~{
  .x %>%
...
results_2018 <-
  raw_2018 %>% 
  separate(filename, into = c("sample", "trash", "leaf")) %>% 
  select(-trash) %>% 
  mutate(cultivar = str_extract(sample, "\\D"),
         plant_num = str_extract(sample, "\\d+"),
         leaf_area_px = undamaged + damaged,
         #leaf_area_mm2 = leaf_area_px * ?????,
         percent_damage = damaged/leaf_area_px * 100) %>% 
  select(sample, cultivar, plant_num, leaf, everything()) %>% 
  
  # Next, combine a's and b's---multiple scans of same plant.
  mutate(leaf = case_when(str_detect(sample, "a$") & leaf != "T" ~ paste0(leaf, "a"),
                          str_detect(sample, "b$") & leaf != "T" ~ paste0(leaf, "b"),
                          TRUE ~ leaf),
         sample = str_remove(sample, "[a-b]$"))

raw_2017 <-
  map2_dfr(.x = dflist_2017, .y = names(dflist_2017), ~{
  .x %>% 
    select(-matches("X1")) %>% 
    gather(-Category, key = filename, value = pixels) %>% 
    spread(key = Category, value = pixels)
...
results_2017 <-
  raw_2017 %>% 
  separate(filename, into = c("trash", "plant_num", "trash2", "leaf")) %>% 
  select(-trash, -trash2) %>% 
  add_column(cultivar = "Q") %>% 
  mutate(sample = paste0(cultivar, plant_num),
         leaf_area_px = undamaged + damaged,
         # leaf_area_mm2 = leaf_area_px * ????,
         percent_damage = damaged/leaf_area_px * 100) %>% 
  select(sample, cultivar, plant_num, leaf, everything())
...
twister_leaf_2018 <-
  results_2018 %>%
  filter(leaf == "T") %>%
  select(sample, twister_damage = percent_damage)

summary_2018 <-
  results_2018 %>% 
  group_by(cultivar, sample) %>% 
...
```{r}
treatment_2018_raw <-
  readxl::read_excel(here('data', 'raw', '2018 density experiment.xlsx')) %>%
  janitor::clean_names()
...
treatment_2018 <- 
  ...
  mutate(cultivar = str_extract(cultivar, "^."),
         sample = paste0(cultivar, bag_id),
         density_start = initial_number_of_leafhoppers/number_shoots,
         density_end = final_number_of_leafhoppers/number_shoots,
         plant_num = as.character(bag_id)) %>% 
  select(sample, cultivar, plant_num, density_treatment, density_start, density_end)
...
```{r}
treatment_2017_raw <-
  read_csv(here('data', 'raw', '2017 manipulative experiment treatment data.csv')) %>% 
  janitor::clean_names()

treatment_2017 <- 
  treatment_2017_raw %>% 
  add_column(cultivar = "Q") %>% 
  mutate(sample = paste0(cultivar, plant)) %>%
  mutate(density_start = leafhoppers_start/leaves,
         density_end = leafhoppers_end/leaves) %>% 
  select(cultivar, sample, plant_num = plant, density_treatment = density, density_start, density_end) %>% 
...
# Relationship between start and end density

```{r}
# treatment_data_2018 <- read_rds(here('data', 'cleaned', '2018_treatment_data.rds'))
ggplot(treatment_data_2018, aes(x = density_start, y = density_end, color = cultivar)) + 
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)

m <- lm(density_end ~ density_start*cultivar, data = treatment_data_2018)
car::Anova(m)
summary(m)
```
```{r}
# treatment_data_2017 <- read_rds(here('data', 'cleaned', '2017_treatment_data.rds'))
ggplot(treatment_data_2017, aes(x = density_start, y = density_end)) +
  geom_jitter(alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE)

m2 <- lm(density_end ~ density_start, data = treatment_data_2017)
summary(m2)
car::Anova(m2)
```


Relationship is maybe a little different between cultivars.