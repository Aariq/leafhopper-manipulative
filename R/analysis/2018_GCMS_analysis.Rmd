...
```{r}
gc_jgy <- gc_wide.logscale %>% 
  filter(cultivar == "J")
gc_lj <- gc_wide.logscale %>% 
  filter(cultivar == "L")
```
...
rda_jgy_t <- 
  rda(gc_jgy %>% filter(!is.na(twister_damage)) %>% select(-metavars) ~ twister_damage,
      data = gc_jgy %>% filter(!is.na(twister_damage)))
anova(rda_jgy_t)
```
...
```{r}
rda_lj_dam$CCA$v %>% as_tibble(rownames = "compound") %>% arrange(desc(abs(RDA1)))
```
