...
lc_lm_2017 <-
  ...
  group_by(compound) %>% 
  nest() %>% 
  mutate(lm = map(data, ~lm(conc_ug_mg ~ mean_percent_damage, data = .x))) %>% 
  mutate(lm.tidy = map(lm, ~broom::tidy(.))) %>%
  unnest(lm.tidy, .preserve = c(data, lm)) %>% 
...
lc_plot.2017 <-
  ...
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, data = lc_plotdata.2017 %>% filter(compound == "Epigallocatechin gallate"), color = "black", size = 0.5, linetype = 1) +
