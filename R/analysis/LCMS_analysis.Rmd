...
ggplot(lc2017_tidy, aes((conc_ug_mg))) +
  geom_histogram(bins = 5) +
  facet_wrap(~compound, scales = "free_x")

resid_m <- lm(log(conc_ug_mg) ~ density_end * compound, data = lc2017_tidy)
shapiro.test(resid_m$residuals)
qqnorm(resid_m$residuals);qqline(resid_m$residuals)
```
...
lc2017_tidy <- 
  ...
  mutate(conc_ug_mg = ifelse(is.na(conc_ug_mg), 0.001, conc_ug_mg))

lc2017_2 <-
  lc2017 %>% 
  mutate_all(~ifelse(is.na(.), 0.001, .)) %>% 
  # mutate_at(vars(Caffeine:Theobromine), log) %>% 
