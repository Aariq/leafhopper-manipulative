...
lc_lm_2017 <-
  ...
  group_by(compound) %>% 
  nest() %>% 
  mutate(linear = map(data, ~lm(conc_ug_mg ~ mean_percent_damage, data = .x)),
         null = map(data, ~lm(conc_ug_mg ~ 1, data = .x)),
         step = map(data, ~ chngptm(conc_ug_mg ~ 1, ~ mean_percent_damage,
                                    family = "gaussian", type = "step", data = .x, 
                                    var.type = "bootstrap", ci.bootstrap.size = 100)),
         hinge = map(data, ~chngptm(conc_ug_mg ~ 1, ~ mean_percent_damage,
...
- Caffeine: linear
- Epicatechin gallate: null (step threshold overlaps zero)
- Epigallocatechin gallate: linear
- Theobromine: null (step threshold overlaps zero)
...
lc_lm_2017 <-
  lc_lm_2017 %>% 
  ungroup() %>% 
  filter(compound == "Caffeine" & type == "linear" |
           compound == "Epicatechin gallate" & type == "null" |
           compound == "Epigallocatechin gallate" & type == "linear" |
           compound == "Theobromine" & type == "null")
```


In all cases the linear model was a better fit than quadratic, so I'm just going with linear.

```{r}
full_join(lc_biomarkers_2017, lc_lm_2017)
```
...
lc_plot.2017 <-
  ggplot(lc_plotdata.2017, aes(x = mean_percent_damage, y = conc_ug_mg)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, data = lc_plotdata.2017 %>% filter(compound %in% c("Epigallocatechin gallate", "Caffeine")), color = "black", size = 0.5, linetype = 1) +
...
rda_dam_jgy <- 
  rda(lc_jgy %>% select(Caffeine:Theobromine) ~ mean_percent_damage,
      data = lc_jgy)
```
...
```{r}
MVA.synt(rda_dens_jgy)
MVA.synt(rda_dam_jgy)
```
...
rda_dam_lj <- 
  rda(lc_lj %>% select(Caffeine:Theobromine) ~ mean_percent_damage,
      data = lc_lj)
```
