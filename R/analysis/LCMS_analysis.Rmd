...
lc2017_tidy <- 
  lc2017_tidy %>% 
  filter(!compound %in% c("Catechin gallate", "Gallic acid", "Theophylline")) %>% #not detected in any sample
  mutate(conc_ug_mg = ifelse(is.na(conc_ug_mg), 0.0001, conc_ug_mg))

lc2017_2 <-
  lc2017 %>% 
  mutate_all(~ifelse(is.na(.), 0.0001, .)) %>% 
  select(-`Catechin gallate`, -`Gallic acid`, -`Theophylline`) #all zeroes

# min(lc2018_tidy$conc_ug_mg, na.rm = TRUE)
lc2018_tidy <-
  lc2018_tidy %>% 
  filter(!compound %in% c("Catechin gallate", "Gallic acid", "Theophylline")) %>% #all zeros for all samples
  mutate(conc_ug_mg = ifelse(is.na(conc_ug_mg), 0.0001, conc_ug_mg)) 

lc2018_2 <- 
  lc2018 %>% 
  mutate_all(~ifelse(is.na(.), 0.0001, .)) %>% 
  select(-`Catechin gallate`, -Theophylline, -`Gallic acid`) #all zeroes
```
...
lc_lm_2017 <-
  ...
  filter(compound == "Caffeine" & type == "null" |
           compound == "Epicatechin gallate" & type == "step" |
           compound == "Epigallocatechin gallate" & type == "linear" |
           compound == "Theobromine" & type == "step")
```
...
lc_plot.2017 <-
  ...
  geom_smooth(method = "lm",
              ...
              data = lc_plotdata.2017 %>% 
                filter(compound %in% c("Epigallocatechin gallate")),
