...
dens_univar <-
  ...
  mutate(null = map(data, ~lm(log.RPA ~ 1, data = .x)),
         ...
         step = map(data, ~chngptm(log.RPA ~ 1, ~ density_end, family = "gaussian", type = "step", data = .x, var.type="bootstrap", ci.bootstrap.size=100)),
         hinge = map(data, ~chngptm(log.RPA ~ 1, ~ density_end, family = "gaussian", type = "hinge", data = .x))) %>% 
  gather(-compound, -data, key = type, value = model) %>% 
  mutate(fit = ifelse(type %in% c("null", "line"), model, map(model, ~.x$best.fit))) %>% 
  arrange(compound) %>% 
...
```{r}
dens_plotdata <- 
  full_join(dens_biomarkers, dens_univar_out, by = "compound") %>% 
...
ggplot(dens_plotdata, aes(x = density_end, y = log.RPA)) +
  ...
  #line
  geom_smooth(method = "lm",se = FALSE, color = "black", data = dens_justlm) +
