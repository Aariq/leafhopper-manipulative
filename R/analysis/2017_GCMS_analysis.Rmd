...
dens_plot <-
  ggplot(dens_data, aes(x = density_end, y = RPA, color = Compound)) +
  geom_point() +
...
```{r}
dam_data <- gc_tidy %>% filter(Compound %in% pull(dam_biomarkers, compound))

dam_plot <-
  ggplot(dam_data, aes(x = twister_damage, y = RPA, color = Compound)) +
  geom_point() +
  facet_wrap(~Compound, scale = "free_y") +
  theme_bw() +
  theme(legend.position = "none") +
  labs(x = "% Damage to DCSE leaf",
       y = "Relative peak area")
dam_plot
```
