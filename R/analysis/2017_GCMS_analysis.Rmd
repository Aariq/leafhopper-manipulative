...
Should I remove Q2?  This was the plant that had a large amount of damage for the # of leafhoppers.  Probably should be consistent across analyses.  Like

"One sample (plant 2) was removed due to a large amount of damage for the leafhopper density.  In addition to that, another sample (plant 8) was removed from the GCMS dataset due to being an outlier as detected by hdoutliers"
...
rda_dens <- 
  rda(gc_wide.log %>% select(-metavars) ~ density_end,
      data = gc_wide.log)
rda_dam <- 
  rda(gc_wide.log %>% select(-metavars) ~ log(twister_damage),
      data = gc_wide.log)
```
...
yeesh, only 8.36% of variance explained by twister leaf damage, 9.67% explained by ending density
...
marginally significant effect of leaf damage on volatiles. Significant effect of ending density.
...
PLS by twister leaf damage is the best model (highest R2Y and Q2Y and lowest RMSEE).  I'll use that for future analyses.  The model with  log-transformed twister damage has lower RMSEE, so I might want to use that.
