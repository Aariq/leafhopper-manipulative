...
unks <- annotations2 %>% filter(is.na(cas)) %>% arrange(meanRI) %>% add_column(unk_name = 1:nrow(.))
...
```{r}
rda_dam <- rda(chemdata ~ log(gc_wide.logscale$mean_percent_damage))
```
...
```{r}
rda_dam <- rda(chemdata ~ log(gc_wide.logscale$twister_damage))
```
...
Marginally significant? Not sure if should report biomarkers. (not significant if not log-transformed)
...
dens_univar <-
  ...
  mutate(null = map(data, ~lm(RPA ~ 1, data = .x)),
         line = map(data, ~lm(RPA ~ density_end, data = .x)),
         step = map(data, ~chngptm(RPA ~ 1, ~ density_end,
                                   family = "gaussian", type = "step", data = .x, var.type="bootstrap", ci.bootstrap.size=100)),
         hinge = map(data, ~chngptm(RPA ~ 1, ~ density_end, 
...
- Correlation with damage RDA (if I decide it's significant)

```{r}
#TODO make supplemental table
```
...
dam_univar <-
  ...
  #fit null, linear, step, and hing models
  mutate(null = map(data, ~lm(RPA ~ 1, data = .x)),
         line = map(data, ~lm(RPA ~ log(twister_damage), data = .x)),
         step = map(data, ~chngptm(RPA ~ 1, ~ log(twister_damage),
                                   family = "gaussian", type = "step", data = .x, var.type = "bootstrap", ci.bootstrap.size = 100)),
         hinge = map(data, ~chngptm(RPA ~ 1, ~ log(twister_damage), 
                                    family = "gaussian", type = "hinge", data = .x))) %>% 
  gather(-compound, -data, key = type, value = model) %>% 
  #extract winning glm from step and hing models
  mutate(fit = ifelse(type %in% c("step", "hinge"), map(model, ~.x$best.fit), model)) %>% 
...
dam_univar2 <- 
  ...
  #set AIC to very high if the threshold CI overlaps zero
  mutate(AIC = if_else(chngpt.lower.CI > 0 | is.na(chngpt.lower.CI), AIC, Inf)) %>%
  mutate(AIC = if_else(chngpt.upper.CI < max(log(gc_tidy$twister_damage)) | is.na(chngpt.upper.CI), AIC, Inf)) %>% 
  group_by(compound) %>%
  # choose the best AIC as described above
  filter(AIC == bestAIC(AIC))
```
...
dam_univar_out <-
  dam_univar2 %>% 
  mutate(estimate = case_when(type == "step"  ~ `I(log(twister_damage)>chngpt)` - `(Intercept)`,
                              type == "hinge" ~ `(log(twister_damage)-chngpt)+`,
                              type == "line"  ~ map_dbl(fit, ~coef(.x)[2]))) %>% 
  select(compound, type, estimate, threshold = chngpt)
```
...
```{r}
dens_plot <- 
  ggplot(dens_plotdata, aes(x = density_end, y = RPA)) +
...
```{r}
dam_plot <- 
  ggplot(dam_plotdata, aes(x = log(twister_damage), y = RPA)) +
  geom_point(alpha = 0.5) +
  geom_segment(aes(x = 0,
                 xend = chngpt,
                 y = `(Intercept)`,
  ...
  #step
  geom_segment(aes(x = chngpt,
                   xend = max(log(gc_tidy$twister_damage)),
                   y = `(Intercept)`+`I(log(twister_damage)>chngpt)`, 
                   yend = `(Intercept)`+`I(log(twister_damage)>chngpt)`), color = "black") +
  #hinge
  geom_segment(aes(x = chngpt,
                   xend = max(log(gc_tidy$twister_damage)),
                   y = `(Intercept)`,
                   yend = `(Intercept)` + `(log(twister_damage)-chngpt)+` * (max(log(gc_tidy$twister_damage)) - chngpt))) +
  #line
