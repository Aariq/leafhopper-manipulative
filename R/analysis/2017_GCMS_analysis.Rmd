...
**UPDATE**: 6.61% when log-transformed leaf damage used
...
```
~~7.94%~~
**UPDATE**: 7.67% when log-transformed twiter damage used
...
dam_univar2 <- 
  ...
  #set AIC to very high if the threshold CI overlaps zero
  mutate(AIC = if_else(chngpt.lower.CI > 0 | is.na(chngpt.lower.CI), AIC, Inf)) %>%
  mutate(AIC = if_else(chngpt.upper.CI < max(gc_tidy$log.twister) | is.na(chngpt.upper.CI), AIC, Inf)) %>% 
  group_by(compound) %>%
  # choose the best AIC as described above
  filter(AIC == bestAIC(AIC))
```
...
dam_univar_out <-
  dam_univar2 %>% 
  mutate(estimate = case_when(type == "step"  ~ `I(log.twister>chngpt)` - `(Intercept)`,
