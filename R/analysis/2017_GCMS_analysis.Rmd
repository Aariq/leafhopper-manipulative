---
title: "Leafhopper 2017 manipulative experiment analysis"
output: html_document
---
# To Do:

- ~~Arcsin sqrt transform % damage data (a common transformation for percentage data to make normal)~~ (doesn't help)
- Wait, do I have to do a log-transformation?  What happens if I don't?
- Remove compounds only found in 3 or fewer samples.
- This is **not** compositional data (because I care about sums and ratios), so don't do clr transformation

# NOTE: 

The image analysis for these data was done using a classifier trained on a different cultivar.  Just by visually inspecting the images, it didn't do a super great job.  Also, there was a lot of "hopperburn" that threw off the damage measurments.  If I get a chance, I should train a new classifier on a random sample of these leaves and re-run PLS regression with the new damage data.
...
gc_wide.ones <- read_rds(here("data", "2017", "cleaned", "2017_GCMS_wide.rds"))
gc_tidy.zeroes <- read_rds(here("data", "2017", "cleaned", "2017_GCMS_tidy.rds"))

leafdamage <- read_rds(here("data", "2017", "cleaned", "leaf damage.rds"))
```
