...
See if I can get confidence intervals for changepoint.  If they include zero, I should not include them
...
dens_data <-
  ...
  slice(1:9) %>% 
  unnest(data) %>% 
  mutate_at(vars("(Intercept)", "I(density_end>chngpt)", "chngpt"), ~ifelse(step.p.value > 0.05, NA, .)) %>% 
  left_join(gc_tidy) %>% 
